...
function handleStartClick() {
    if (game.getStatus() === "lose" || game.getStatus() === "win") game.restart();
    game.start();
    renderBoard();
    updateUI();
}
 ...
 */ class Game {
   ...
   */ constructor(initialState){
        ...
        const initialBoardState = initialState ? initialState.map((line)=>[
                ...line
            ]) : [
            [
                0,
    ...
    moveLeft() {
        ...
        for (const row of this.board){
            ...
            if (row.join(",") !== newRow.join(",")) boardChanged = true;
        }
        if (boardChanged) {
            this.board = newBoard;
            this.addRandomTile();
   ...
   */ getStatus() {
        if (this._hasWon()) return "win";
        if (!this._hasMovesLeft()) return "lose";
        return this.status;
    }
    /**
   * Starts the game.
   ...
   */ restart() {
    ...
    getEmptyCells() {
        ...
        for(let i = 0; i < 4; i++){
            ...
            if (this.board[i][j] === 0) emptyCells.push({
                line: i,
                column: j
            });
        }
    ...
    _transposeBoard(board) {
        ...
        for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++)newBoard[j].push(board[i][j]);
        return newBoard;
    }
    _hasWon() {
